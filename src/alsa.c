...
//    printf("routing snk %d: %s lr_num %d\n", count, elem->name, elem->lr_num);
//    char side;
//    if (sscanf(elem->name, "Master %d%c", &elem->lr_num, &side) == 2) {
//      elem->lr_num = elem->lr_num * 2 - (side == 'L');
//    } else {
//      elem->lr_num = get_num_from_string(elem->name);
//    }
//    printf("routing snk %d: %s lr_num %d\n", count, elem->name, elem->lr_num);
