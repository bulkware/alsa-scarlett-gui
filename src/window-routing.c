...
static void routing_preset_link(
  ...
  for (snk_idx = 0;
    ...
    if (r_snk->elem->port_category == snk_port_category)
      break;
  }
...
static void make_routing_alsa_elem(struct routing_snk *r_snk) {
  ...
  if (r_snk->elem->port_category == PC_DSP) {
  ...
  } else if (r_snk->elem->port_category == PC_PCM) {
